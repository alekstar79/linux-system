...
const Dock = memo(function Dock() {
  ...
  const processedBounceIds = useRef(new Set());

  // Bounce animation handler
  useEffect(() => {
    const itemsToBounce = dockItems.filter(
      (d) => d.bounce && !processedBounceIds.current.has(d.appId)
    );

    if (itemsToBounce.length > 0) {
      const newBouncingAppIds = itemsToBounce.map((d) => d.appId);

      // Add to bouncing state for animation
      setBouncingItems((prev) => new Set([...prev, ...newBouncingAppIds]));

      // Mark as processed
      newBouncingAppIds.forEach((id) => processedBounceIds.current.add(id));

      // Dispatch action to turn off the bounce flag in the central store
      // This is crucial to prevent the loop.
      dispatch({ type: 'ACKNOWLEDGE_BOUNCE', appIds: newBouncingAppIds });

      // Set a timer to remove the visual bounce effect from the UI
      const timer = setTimeout(() => {
        setBouncingItems((prev) => {
          const next = new Set(prev);
          newBouncingAppIds.forEach((id) => next.delete(id));
          return next;
        });
        // Clean up processed IDs after animation
        newBouncingAppIds.forEach((id) => processedBounceIds.current.delete(id));
