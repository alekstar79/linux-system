...
export default function CodeEditor() {
  ...
  const buttonClass = "flex items-center gap-1.5 px-2.5 py-1 rounded text-xs transition-colors border border-transparent hover:bg-[var(--bg-hover)] hover:border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]";

  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--bg-window)' }}>
      {/* Menu Bar */}
      <div className="flex items-center gap-1 px-2 shrink-0" style={{ height: 36, background: 'var(--bg-titlebar)', borderBottom: '1px solid var(--border-subtle)' }}>
        <button onClick={newFile} className={buttonClass} title="New File">
          <Plus size={14} /><span className="text-xs">New</span>
        </button>
        <button onClick={() => setShowSidebar((v) => !v)} className={buttonClass} title="Toggle Sidebar">
          <FolderOpen size={14} /><span className="text-xs">Explorer</span>
        </button>
        <button onClick={() => setShowFindBar((v) => !v)} className={buttonClass} title="Find (Ctrl+F)">
          <Search size={14} /><span className="text-xs">Find</span>
        </button>
        <button onClick={saveFile} className={buttonClass} title="Save (Ctrl+S)">
